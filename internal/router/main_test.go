...
// TestQualityFloorAboveBarFreesWithinGrace covers fix #2 (a): every above-target
// worker is momentarily full, but one frees a slot WITHIN the grace — the request
// must be served above the floor, not downgraded.
func TestQualityFloorAboveBarFreesWithinGrace(t *testing.T) {
	withQualityFloorWait(t, 500*time.Millisecond)
	reg := newTestRegistry()
	big := registerQ(t, reg, "big", 10, 1)  // above-bar (target 7)
	tiny := registerQ(t, reg, "tiny", 2, 1) // below-bar
	r := &Router{registry: reg}
	cands := []*Backend{big, tiny}

	// Saturate the above-bar worker; the below-bar one stays free.
	bigSlot, ok := reg.tryAcquireSlot("big")
	if !ok {
		t.Fatal("could not saturate big")
	}
	// Free big shortly — well within the grace — so the floor wait catches it.
	go func() {
	...
	if got.ID != "big" {
		t.Fatalf("a slot freed within the grace: want above-bar big, got %s", got.ID)
	}
	if downgraded {
		t.Fatal("served above the floor must not report a downgrade")
	}
	reg.releaseSlot(slot)
}

// TestQualityFloorDowngradesPastGrace covers fix #2 (b): the above-target worker
// stays full past the grace, so the request spills to a below-bar worker and is
// flagged as a downgrade.
func TestQualityFloorDowngradesPastGrace(t *testing.T) {
	withQualityFloorWait(t, 80*time.Millisecond)
	reg := newTestRegistry()
	big := registerQ(t, reg, "big", 10, 1)  // above-bar
	tiny := registerQ(t, reg, "tiny", 2, 1) // below-bar, free
	r := &Router{registry: reg}
	cands := []*Backend{big, tiny}

	if _, ok := reg.tryAcquireSlot("big"); !ok { // saturate big and keep it full
		t.Fatal("could not saturate big")
	}
	start := time.Now()
	got, slot, downgraded, err := r.pickAndAcquireWithFloor(context.Background(), cands, 7)
	...
	if got.ID != "tiny" {
		t.Fatalf("above-bar stayed full past grace: want below-bar tiny, got %s", got.ID)
	}
	if !downgraded {
		t.Fatal("serving below the floor after the grace must report a downgrade")
	}
	if elapsed := time.Since(start); elapsed < qualityFloorWait {
		t.Fatalf("downgraded before waiting the full grace: %s < %s", elapsed, qualityFloorWait)
	}
	reg.releaseSlot(slot)
}

// TestQualityFloorNoAboveBarImmediate covers fix #2 (c): NO worker clears the
// target, so there is nothing to wait for — the best below-bar worker is served
// immediately with no extra grace and no downgrade flag (it's the best we have).
func TestQualityFloorNoAboveBarImmediate(t *testing.T) {
	withQualityFloorWait(t, 5*time.Second) // large; must NOT be consumed
	reg := newTestRegistry()
	mid := registerQ(t, reg, "mid", 5, 1)   // below target 9
	tiny := registerQ(t, reg, "tiny", 2, 1) // below target 9
	r := &Router{registry: reg}
	...
	if got.ID != "mid" {
		t.Fatalf("no above-bar worker: want best below-bar (mid, ranked first), got %s", got.ID)
	}
	if downgraded {
		t.Fatal("no sufficient worker exists at all — that's not a downgrade event")
	}
	if elapsed := time.Since(start); elapsed > time.Second {
		t.Fatalf("waited the grace when no above-bar worker exists: %s", elapsed)
	}
	reg.releaseSlot(slot)
}

// TestQualityFloorTargetZeroUnchanged covers fix #2 (d): target=0 means everyone
// is above-bar — behaviour must be identical to plain pickAndAcquire (immediate
// best free slot, spill, no downgrade flag, no extra wait).
func TestQualityFloorTargetZeroUnchanged(t *testing.T) {
	withQualityFloorWait(t, 5*time.Second) // must NOT be consumed
	reg := newTestRegistry()
	a := registerQ(t, reg, "a", 2, 1)
	b := registerQ(t, reg, "b", 10, 1)
	r := &Router{registry: reg}
	cands := []*Backend{a, b}
	...
	// Saturate the first; target=0 must spill to the next with no downgrade flag.
	if _, ok := reg.tryAcquireSlot("a"); !ok {
		t.Fatal("could not saturate a")
	}
	start := time.Now()
	...
	if elapsed := time.Since(start); elapsed > time.Second {
		t.Fatalf("target=0 must not consume the floor grace: %s", elapsed)
	}
	reg.releaseSlot(slot)
}
