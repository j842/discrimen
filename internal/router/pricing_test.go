...
// at different prices, and a model that exists only under its vendor prefix.
const testPriceJSON = `{
 "source": "test",
...
func TestSeedPricesOnlyFillsBlanks(t *testing.T) {
	...
	t.Run("no match seeds nothing", func(t *testing.T) {
		reg := BackendRegistration{Model: "some-local-gguf", Provider: providerLocal}
		if filled := seedPricesWith(tbl, &reg, priceStated{}); len(filled) != 0 || reg.InputPricePerMtok != 0 {
			t.Errorf("a local worker must stay free: %v %+v", filled, reg)
		}
	})
